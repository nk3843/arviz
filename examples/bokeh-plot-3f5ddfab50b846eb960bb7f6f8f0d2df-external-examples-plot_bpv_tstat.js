(function() {
  var fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      var force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      var element = document.getElementById("84ccf00a-ea43-4486-bc51-b03ae37a8dc4");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid '84ccf00a-ea43-4486-bc51-b03ae37a8dc4' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error(url) {
          console.error("failed to load " + url);
        }
    
        for (let i = 0; i < css_urls.length; i++) {
          const url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        const hashes = {"https://cdn.bokeh.org/bokeh/release/bokeh-2.3.2.min.js": "XypntL49z55iwGVUW4qsEu83zKL3XEcz0MjuGOQ9SlaaQ68X/g+k1FcioZi7oQAc", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.3.2.min.js": "bEsM86IHGDTLCS0Zod8a8WM6Y4+lafAL/eSiyQcuPzinmWNgNO2/olUF0Z2Dkn5i", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.3.2.min.js": "TX0gSQTdXTTeScqxj6PVQxTiRW8DOoGVwinyi1D3kxv7wuxQ02XkOxv0xwiypcAH"};
    
        for (let i = 0; i < js_urls.length; i++) {
          const url = js_urls[i];
          const element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.async = false;
          element.src = url;
          if (url in hashes) {
            element.crossOrigin = "anonymous";
            element.integrity = "sha384-" + hashes[url];
          }
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      var js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.3.2.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.3.2.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.3.2.min.js"];
      var css_urls = [];
      
    
      var inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            var fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  var docs_json = '{"a959a65f-5c07-45a6-8907-3b83d15ef114":{"defs":[],"roots":{"references":[{"attributes":{"axis":{"id":"15537"},"ticker":null},"id":"15540","type":"Grid"},{"attributes":{"data_source":{"id":"15564"},"glyph":{"id":"15565"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"15566"},"view":{"id":"15568"}},"id":"15567","type":"GlyphRenderer"},{"attributes":{},"id":"15531","type":"DataRange1d"},{"attributes":{"toolbars":[{"id":"15555"}],"tools":[{"id":"15545"},{"id":"15546"},{"id":"15547"},{"id":"15548"},{"id":"15549"},{"id":"15550"},{"id":"15551"},{"id":"15552"}]},"id":"15614","type":"ProxyToolbar"},{"attributes":{},"id":"15551","type":"SaveTool"},{"attributes":{"line_alpha":0,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"15570","type":"Line"},{"attributes":{"children":[{"id":"15615"},{"id":"15613"}]},"id":"15616","type":"Column"},{"attributes":{},"id":"15548","type":"WheelZoomTool"},{"attributes":{"formatter":{"id":"15576"},"major_label_policy":{"id":"15577"},"ticker":{"id":"15538"}},"id":"15537","type":"LinearAxis"},{"attributes":{},"id":"15533","type":"LinearScale"},{"attributes":{},"id":"15577","type":"AllLabels"},{"attributes":{},"id":"15576","type":"BasicTickFormatter"},{"attributes":{"source":{"id":"15569"}},"id":"15573","type":"CDSView"},{"attributes":{"data":{},"selected":{"id":"15609"},"selection_policy":{"id":"15608"}},"id":"15590","type":"ColumnDataSource"},{"attributes":{},"id":"15529","type":"DataRange1d"},{"attributes":{"children":[[{"id":"15528"},0,0]]},"id":"15613","type":"GridBox"},{"attributes":{"overlay":{"id":"15554"}},"id":"15549","type":"LassoSelectTool"},{"attributes":{"data_source":{"id":"15590"},"glyph":{"id":"15591"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"15592"},"view":{"id":"15594"}},"id":"15593","type":"GlyphRenderer"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"15583"},"selection_policy":{"id":"15582"}},"id":"15564","type":"ColumnDataSource"},{"attributes":{"active_multi":null,"tools":[{"id":"15545"},{"id":"15546"},{"id":"15547"},{"id":"15548"},{"id":"15549"},{"id":"15550"},{"id":"15551"},{"id":"15552"}]},"id":"15555","type":"Toolbar"},{"attributes":{},"id":"15579","type":"BasicTickFormatter"},{"attributes":{},"id":"15583","type":"Selection"},{"attributes":{"toolbar":{"id":"15614"},"toolbar_location":"above"},"id":"15615","type":"ToolbarBox"},{"attributes":{},"id":"15609","type":"Selection"},{"attributes":{"callback":null},"id":"15552","type":"HoverTool"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"15571","type":"Line"},{"attributes":{},"id":"15550","type":"UndoTool"},{"attributes":{"axis":{"id":"15541"},"dimension":1,"ticker":null},"id":"15544","type":"Grid"},{"attributes":{},"id":"15582","type":"UnionRenderers"},{"attributes":{"below":[{"id":"15537"}],"center":[{"id":"15540"},{"id":"15544"},{"id":"15588"}],"height":500,"left":[{"id":"15541"}],"output_backend":"webgl","renderers":[{"id":"15567"},{"id":"15572"},{"id":"15593"}],"title":{"id":"15595"},"toolbar":{"id":"15555"},"toolbar_location":null,"width":500,"x_range":{"id":"15529"},"x_scale":{"id":"15533"},"y_range":{"id":"15531"},"y_scale":{"id":"15535"}},"id":"15528","subtype":"Figure","type":"Plot"},{"attributes":{"label":{"value":"bpv=0.53"},"renderers":[{"id":"15572"}]},"id":"15589","type":"LegendItem"},{"attributes":{},"id":"15585","type":"Selection"},{"attributes":{},"id":"15584","type":"UnionRenderers"},{"attributes":{"formatter":{"id":"15579"},"major_label_policy":{"id":"15580"},"ticker":{"id":"15542"}},"id":"15541","type":"LinearAxis"},{"attributes":{"overlay":{"id":"15553"}},"id":"15547","type":"BoxZoomTool"},{"attributes":{"items":[{"id":"15589"}]},"id":"15588","type":"Legend"},{"attributes":{"source":{"id":"15564"}},"id":"15568","type":"CDSView"},{"attributes":{"fill_alpha":0.5,"fill_color":"lightgrey","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"syncable":false,"xs_units":"screen","ys_units":"screen"},"id":"15554","type":"PolyAnnotation"},{"attributes":{"fill_color":{"value":"#2a2eec"},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"15591","type":"Circle"},{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"15566","type":"Line"},{"attributes":{"source":{"id":"15590"}},"id":"15594","type":"CDSView"},{"attributes":{},"id":"15608","type":"UnionRenderers"},{"attributes":{"data":{},"selected":{"id":"15585"},"selection_policy":{"id":"15584"}},"id":"15569","type":"ColumnDataSource"},{"attributes":{"bottom_units":"screen","fill_alpha":0.5,"fill_color":"lightgrey","left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","syncable":false,"top_units":"screen"},"id":"15553","type":"BoxAnnotation"},{"attributes":{},"id":"15580","type":"AllLabels"},{"attributes":{},"id":"15545","type":"ResetTool"},{"attributes":{},"id":"15546","type":"PanTool"},{"attributes":{},"id":"15535","type":"LinearScale"},{"attributes":{"data_source":{"id":"15569"},"glyph":{"id":"15570"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"15571"},"view":{"id":"15573"}},"id":"15572","type":"GlyphRenderer"},{"attributes":{},"id":"15538","type":"BasicTicker"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#2a2eec"},"line_alpha":{"value":0.1},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"15592","type":"Circle"},{"attributes":{"text":"y / y","text_font_size":"15pt"},"id":"15595","type":"Title"},{"attributes":{},"id":"15542","type":"BasicTicker"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"15565","type":"Line"}],"root_ids":["15616"]},"title":"Bokeh Application","version":"2.3.2"}}';
                  var render_items = [{"docid":"a959a65f-5c07-45a6-8907-3b83d15ef114","root_ids":["15616"],"roots":{"15616":"84ccf00a-ea43-4486-bc51-b03ae37a8dc4"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    var attempts = 0;
                    var timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (var i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();